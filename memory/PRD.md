...
- [x] Pembayaran Tim = sistem entri pembayaran (17 Feb 2026): `TeamPayments.jsx` ditulis ulang total. Tombol "Buat Pembayaran" (`pe-create-btn`) buka form: multi-select proyek Siap Dibayar via Popover+Checkbox (`pe-project-multiselect`, `pe-project-check-{id}`), Kategori Pembayaran dropdown periode "Tanggal 5 s/d 19" (value 5-19) & "Tanggal 20 s/d 4" (value 20-4) (`pe-period-select`), kalkulasi otomatis (kartu total nilai proyek/kasbon/diterima, testid `pe-total-*`), tabel rincian anggota (nama, diterima setelah kasbon, status DIBAYAR). Simpan → entri muncul di riwayat (tabel utama, `pe-row-{id}`) dengan Detail dialog & Hapus. Backend: koleksi `payment_entries`, `POST/GET /api/payment-entries` + `DELETE /api/payment-entries/{eid}` (owner/bendahara). POST validasi period & agregasi server-side dari team_payments paid=true.
