...
- [x] Rename tab + batasan hapus + Pembayaran Saya baru (18 Feb 2026): (1) Tab "Riwayat Entri Pembayaran" di-rename "Entry Pembayaran". (2) Tombol Hapus entri hanya untuk owner (frontend `canDelete = role==="owner"` via useAuth; backend DELETE /payment-entries require_role("owner") — bendahara/tim 403). (3) `MyPayments.jsx` ditulis ulang: data dari endpoint baru `GET /api/my-payment-entries` (role tim, per entri berisi bagian user saja: month, period, amount, kasbon, net, received, locations). Tabel: Periode Pembayaran, Total Hasil, Potongan Kasbon, Diterima, Status (DITERIMA/MENUNGGU dari konfirmasi Bendahara), tombol Detail (`mp-detail-btn-{id}`) → dialog "Detail Lokasi Proyek" (`mp-loc-detail`, baris `mp-loc-row-{i}` + TOTAL). Filter bulan tetap (`mp-month-filter`, dari entry.month).
- [x] Kolom Proyek 3 baris + PDF buku kas + lokasi rinci (18 Feb 2026): (1) Riwayat Buku Kas kolom Proyek: nama + jenis pekerjaan (text-sm) + keterangan (text-xs). (2) Detail Entry Pembayaran & Pembayaran Saya: kolom Lokasi Proyek kini nama + jenis (text-xs) + keterangan (text-[11px]) — `compute_entry_details` diperkaya `work_type`+`keterangan`, helper `details_stale()` recompute entri lama. (3) Tombol PDF per proyek di Riwayat Buku Kas (`history-pdf-btn-{id}`) → `GET /api/bukukas/{lid}/pdf` (tim hanya lokasi miliknya, 403 selain itu): tabel Tanggal, Keterangan, kolom terpisah Pemasukan/Operasional/Material/Kasbon (kategori lama non-Material/Kasbon masuk Operasional) + TOTAL + Saldo Akhir. Diverifikasi render visual PDF + screenshot.
...
## Update (Feb 2026) - Kolom Proyek di Tab Proyek Berjalan/Selesai
