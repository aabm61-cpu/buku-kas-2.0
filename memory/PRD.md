...
- [x] Menu Rekap Pembayaran (17 Feb 2026): menu sidebar baru "Rekap Pembayaran" (`/rekap-pembayaran`, owner/bendahara, icon ListChecks) — halaman `RekapPembayaran.jsx` berisi tab Menunggu Pembayaran & Siap Dibayar yang dipindah dari Pembayaran Tim (semua alur tetap: dialog Aksi, lock, tombol Siap Dibayar/Kembali, filter bulan, testid tetap `tp-*`). `TeamPayments.jsx` (menu Pembayaran Tim) kini hanya berisi Rekap Invoice (checkbox multi-proyek + rekap gabungan). PENTING: ListChecks harus diimport di Layout.jsx (sempat error runtime).
